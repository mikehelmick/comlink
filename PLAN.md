...
- **Message ID:** `(conversation_id, replica_id, sequence_number)`. The sequence number is monotonic per replica per conversation. This makes message IDs globally unique without coordination, makes "which message did this come from" trivial, and makes per-sender duplicate detection a simple watermark check.
